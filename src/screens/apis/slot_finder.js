    ...
    return (
        <View>
            <View style={{ marginHorizontal: 20, marginVertical: 15 }}>
                ...
                }} label="enter your pincode" value={pincode} onChangeText={(val) => {
                    setPincode(val);
                    if (pincode.length ==5) { setPincheck(false); }
                }} keyboardType='number-pad' style={style.input} />
                <Button loading={loadingcheck} disabled={pincheck} icon="search-web" labelStyle={style.txt} mode="contained" onPress={() => { searchfunc(pincode) }} style={style.btn}>Search</Button>
                    ...
                    <ScrollView style={[{ marginVertical: 20, elevation: -5, paddingTop: 5, backgroundColor: '#E9DCC9' }]} overScrollMode='auto'>
                        ...
                        {dat.sessions.map((i) => {
                            return (<View key={i.session_id} style={{ elevation: 5, backgroundColor: '#f5f0f6', marginBottom: 5, borderRadius: 25, borderWidth: (i.available_capacity_dose1 == 0 ? (i.available_capacity_dose2 == 0 ? 0 : 2) : 2), borderColor: '#ff9551' }}>
                                ...
                                <Text style={[{
                                    fontSize: 18,
                                    fontWeight: '500', marginLeft: 20, color: '#8f8f8f'
                                }]}>{i.address}</Text>
